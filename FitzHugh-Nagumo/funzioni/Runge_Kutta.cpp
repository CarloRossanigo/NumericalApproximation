...
void step(Real *v, Real h, Real *coef, Real *K, int stadi, int d, Real *y)  //v=vector to sum, h=step, Coef=coefficients a, k=Ki, number of stadium ...
{
    //d=dim mat, y=U(n+1). Need to calculate ki as a cycle between i and d with step function, the value of coefs will be the matrix A at the row i 
...
void calcola_c(int ns, Real *c, Real *A)
{
// caluclate c as sum on rows of A
    for(int k=0; k<ns; k++)
    {
        Real sum=*(A+k*ns);
        for (int j=1; j<ns; j++)
...
void EE(int ns, Real *A, Real *b, Real *c, Real *bc, int *Ind)
{
    ...
    // Explicit Euler   p =1, ns = 1;
    b[0]=1.0;
    calcola_c(ns,c,A);
}
void RK4(int ns, Real *A, Real *b, Real *c, Real *bc, int *Ind)
{
    azzera(ns,A,b,c,bc,Ind); // Put all values to 0 before begin 
    // Runge-Kutta 4  p =4, ns = 4;
    b[0]=b[3]=1.0/6.0;
    b[1]=b[2]=1.0/3.0;
    *(A+4)=*(A+9)=0.5; // Position of the matrix's elements needed 
    *(A+14)=1.0;
    calcola_c(ns,c,A);
}
...
{
    azzera(ns,A,b,c,bc,Ind);
// Implicit Euler  / Radau1, p = 1, ns = 1;
// implicit DIRK
    b[0]=1.0;
    A[0]=1.0;
    calcola_c(ns,c,A);
