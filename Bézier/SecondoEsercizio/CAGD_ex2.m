%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% MATLAB: Code for Bézier Curve Intersection Algorithm
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
clear all;
close all;
...
%% Step 1: Check if intersection exists
...
grid on;
yticks(-2:3);
title('Intersection of two curves: first view','FontSize',24);
hold off;

%% Step 2: Search for the intersection point
figure(); hold on;
...
grid on;
yticks(-2:3);
title('Intersection of two curves: algorithm','FontSize',24);
hold off;

%% Step 3: Calculate intersection point as common point of the two segments
...
fprintf('\n Intersection point: %f %f %f %f ',XI1,YI1,XI2,YI2);
...
%% Local view of the intersection point
text(XI1,YI1,'X');text(XI2,YI2,'O');
title('Local view of the intersection of two curves','FontSize',24);
...
fprintf('\n Values for the first curve (calculated with x): %f %f %f %f %f ',tx1,XI1,YI1,XXI1(1,1),XXI1(1,2));
fprintf('\n Values for the first curve (calculated with y): %f %f %f %f %f ',ty1,XI1,YI1,XXXI1(1,1),XXXI1(1,2));
tx2=(XI2-XQ2(1))/(XQ2(np2)-XQ2(1));ty2=(YI2-YQ2(1))/(YQ2(np2)-YQ2(1));
XXI2=CAGD_casteljau(tx2,P4);XXXI2=CAGD_casteljau(ty2,P4);
fprintf('\n Values for the second curve (calculated with x): %f %f %f %f %f ',tx2,XI2,YI2,XXI2(1,1),XXI2(1,2));
fprintf('\n Values for the second curve (calculated with y): %f %f %f %f %f ',ty2,XI2,YI2,XXXI2(1,1),XXXI2(1,2));
...
grid on;
yticks(-2:3);
title('Intersection of two curves: global view','FontSize',24);
hold off;

%% Step 7: Calculate parameter values
%%
...
test1=1;
% fprintf('\n Intersection point on the first curve: %f %f %d ',tx1,ty1,k);
end;end;end;
% fprintf('\n Verify position on the first curve: %d ',test1);
for k=1:size(t,2)-1
...
test2=1;
%fprintf('\n Intersection point on the second curve: %f %f %d ',tx2,ty2,k);
end;end;end;
% fprintf('\n Verify position on the second curve: %d ',test2);
XXI1x=CAGD_casteljau(tx1,P1);XXI1y=CAGD_casteljau(ty1,P1);
fprintf('\n Intersection point on the first curve (calculated with x): %f %f %f %f %f ',tx1,XI1,YI1,XXI1x(1,1),XXI1x(1,2));
fprintf('\n Intersection point on the first curve (calculated with y): %f %f %f %f %f ',ty1,XI1,YI1,XXI1y(1,1),XXI1y(1,2));
XXI2x=CAGD_casteljau(tx2,P2);XXI2y=CAGD_casteljau(ty2,P2);
fprintf('\n Intersection point on the second curve (calculated with x): %f %f %f %f %f ',tx2,XI2,YI2,XXI2x(1,1),XXI2x(1,2));
fprintf('\n Intersection point on the second curve (calculated with y): %f %f %f %f %f \n',ty2,XI2,YI2,XXI2y(1,1),XXI2y(1,2));
