...
%%
%%   Input : P1 control points matrix
%%           P2 control points matrix
%%           d1,f1 control points parameters
%%           d2,f2 control points parameters
%%           iter number of calls to this procedure
%%
%%   Output :
%%           Q1 new control points matrix
%%           Q2 new control points matrix
...
%%
%%  Check convergence (number of iterations)
%%
iter=iter+1;
itmax=20;
if ( iter > itmax )
    fprintf('\n The scheme does not converge ');
    trectan(xi,xa,yi,ya,'b');
    return
end
%%
%%  Check convergence (convexity)
%%
h1=convex(P1);
h2=convex(P2);
h=max(h1,h2);
tolc=5.e-05;
if ( h < tolc )
    fprintf('\n Convexity convergence h = %f ',h);
    fprintf('\n Iterations: %d ',iter);
    return
end
%%
%%  Convergence in x-axis
%%
toll=1.e-04;
if ( abs(xa-xi) < toll )
    fprintf('\n Convergence in x');
