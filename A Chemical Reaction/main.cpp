...
//definition of the functions fnewton and Jfnewton for Newton's iterations 
void effe_newton(Real *F,Real *X);
void Jeffe_newton(Real *J,Real *X);

// pointers to point ad problem data 
void(*funz_effe)(Real*, Real,Real*);                           // Function of the vector field of the differential equation 
void(*dati)(Real *,Real *,Real *);                          // Initial data of the problem
void(*butcher)(int, Real *,Real *,Real *,Real *, int *);    // Butcher's table coefficients 
void(*Jeffe)(Real*,Real,Real*);                                //jacobian of f

// common variables
Real t,T,h,hnew;
// dimension, problme data e method data 
unsigned long valf;                                // variable for counting the evalution of f 
    const int dim=3;                                           //Dimension of the problem, with a constan of the motus is 3 not 4  
const Real p=4.0;                                           //order of the method with the highest order : lobatto 3c has order 4
Real u[dim];
//initialize butcher's table 
    const int s=3; // number of stadium 
    Real b[s];
    Real bc[s];
    Real c[s];
    Real A[s][s];
...
int main ()
{
    funz_effe=eqdiff_f;                                     //Field function
    dati=dati_iniziali;                                      //Initial data
    butcher=LOBATTO_3C_ORD4;                                //matrix coeeficients of the method 
    Jeffe=jf;                                               //jacobian of f
    butcher(s,A[0],b,c,bc,0);                              // equals 0 of teh vectors in the methods are useless 
